...
class ACO(object):
    ...
    def run(self):
        ...
        while convergence_iterations:
            ...
            ## construct solutions
            routes, route_distances = self.__step__(self.num_ants, self.nodes, self.distances, self.pheromones, self.alpha, self.beta)
    ...
    def __next_node__(self, _from, rem_neighbors, distances, pheromones, alpha, beta):
        ...
        sum_probs = rem_probs.sum()

        if sum_probs != 0.0:
            # picks one node based on their probabilities
